...
# Converts compact durations accepted by humans into GNU date expressions.
since_expr() {
  case "$SINCE" in
...
if [[ "$ERRORS_ONLY" == "1" ]]; then
  FILTER_PATTERN='?ERROR ?Error ?error ?panic ?PANIC ?Exception'
fi

# Fetches and prints CloudWatch log events since START_MS.
fetch_logs() {
