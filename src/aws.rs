...
/// Tiny signed AWS HTTP client for JSON-protocol services.
#[derive(Clone)]
...
impl AwsJsonClient {
    ...
    /// Sends a JSON request to an AWS JSON-protocol service such as DynamoDB.
    pub async fn post_json(&self, service: &str, target: &str, body: Value) -> Result<Value> {
        self.post_json_with_content_type(service, target, "application/x-amz-json-1.0", body)
            .await
    }

    /// Sends a JSON 1.1 request to an AWS JSON-protocol service such as SSM.
    pub async fn post_json_1_1(&self, service: &str, target: &str, body: Value) -> Result<Value> {
        self.post_json_with_content_type(service, target, "application/x-amz-json-1.1", body)
            .await
    }

    /// Sends a JSON request with an explicit AWS JSON content type.
    async fn post_json_with_content_type(
        &self,
        service: &str,
        target: &str,
        content_type: &str,
    ...
    pub async fn post_query(&self, service: &str, params: &[(String, String)]) -> Result<String> {
        let credentials = self
            .credentials
            .as_ref()
            .context("AWS credentials are not available in the environment")?;
